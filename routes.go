...
func initRouter(h handler) {
	...
	r.HandleFunc("/api/cancel", h.homeHandler).Methods("GET")
	r.HandleFunc("/contact-us", h.contactPageHandler).Methods("GET")
	...
	if err != nil {
		log.Println("cannot init server, reason -> %s", err.Error())
		panic("db failure")
	}
}
